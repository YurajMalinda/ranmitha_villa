...
export function RoomsSection() {
    ...
    return (
        <section id="rooms" className="py-20 md:py-28 px-4 md:px-8 bg-white dark:bg-slate-950">
            <div className="max-w-7xl mx-auto">
                <motion.div
                    ...
                    <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-[#2A2018] dark:text-white mb-4">
                        {title.main}
                        <span className="text-[#2E5D4B] dark:text-emerald-400"> {title.highlight}</span>
                    </h2>
                ...
                {loading ? (
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        {Array.from({ length: 2 }).map((_, i) => (
                            <div key={i} className="bg-[#FBF8F3] dark:bg-slate-900 rounded-2xl overflow-hidden shadow-sm animate-pulse">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        {displayRooms.map((room, index) => (
                            <motion.div
                                ...
                                className={`bg-[#FBF8F3] dark:bg-slate-900 rounded-2xl overflow-hidden ${room.featured ? 'ring-2 ring-[#D4784A] shadow-lg' : 'shadow-sm'}`}>
                                ...
                                <div className="relative h-64 overflow-hidden group bg-[#EFF7F3] dark:bg-slate-800">
                                    {room.image && (
                                        <Image
                                            src={room.image}
                                            alt={room.name}
                                    ...
                                    <div className="absolute top-4 left-4 bg-white/95 dark:bg-slate-900/90 backdrop-blur-sm px-4 py-1.5 rounded-full shadow-sm">
                                        <span className="font-bold text-[#2E5D4B] dark:text-emerald-400">{room.price}</span>
                                        <span className="text-gray-500 dark:text-gray-400 text-sm"> /night</span>
                                    </div>
                                </div>

                                <div className="p-6 md:p-8">
                                    <h3 className="text-2xl font-bold text-[#2A2018] dark:text-white mb-1">{room.name}</h3>
                                    ...
                                    <div className="flex flex-wrap gap-3 mb-5">
                                        <div className="flex items-center gap-2 bg-white dark:bg-slate-800 px-3 py-1.5 rounded-lg text-sm text-gray-600 dark:text-gray-300 shadow-sm">
                                            <Maximize className="w-4 h-4 text-[#2E5D4B] dark:text-emerald-400" />
                                            {room.size}
                                        </div>
                                        <div className="flex items-center gap-2 bg-white dark:bg-slate-800 px-3 py-1.5 rounded-lg text-sm text-gray-600 dark:text-gray-300 shadow-sm">
                                            <Users className="w-4 h-4 text-[#2E5D4B] dark:text-emerald-400" />
                                            {room.guests}
                                        </div>
                                        <div className="flex items-center gap-2 bg-white dark:bg-slate-800 px-3 py-1.5 rounded-lg text-sm text-gray-600 dark:text-gray-300 shadow-sm">
                                            <Bed className="w-4 h-4 text-[#2E5D4B] dark:text-emerald-400" />
                                            {room.beds}
                                        </div>
                                    </div>

                                    <p className="text-gray-600 dark:text-gray-400 text-sm leading-relaxed mb-5">{room.description}</p>
                                        ...
                                        <div className="mb-6">
                                            <div className="grid grid-cols-2 gap-2">
                                                {(expandedAmenities.has(room._id)
                                                    ...
                                                    <div key={aIndex} className="flex items-center gap-2">
                                                        <div className="w-4 h-4 rounded-full bg-[#EFF7F3] dark:bg-slate-800 flex items-center justify-center flex-shrink-0">
                                                            <Check className="w-2.5 h-2.5 text-[#2E5D4B] dark:text-emerald-400" />
                                                        </div>
                                            ...
                                            {room.amenities.length > AMENITY_PREVIEW_COUNT && (
                                                <button
                                                    ...
                                                    className="text-xs font-semibold text-[#2E5D4B] dark:text-emerald-400 mt-3 hover:underline"
                                                >
                                                    {expandedAmenities.has(room._id)
                                                        ? 'Show less'
