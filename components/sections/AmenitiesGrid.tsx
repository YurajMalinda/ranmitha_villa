...
export function AmenitiesGrid() {
    ...
    return (
        <section id="amenities" className="py-20 md:py-28 px-4 md:px-8 bg-white dark:bg-slate-950">
            <div className="max-w-6xl mx-auto">
                ...
                {loading && (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12 animate-pulse">
                        {Array.from({ length: 8 }).map((_, i) => (
                            <div key={i} className="flex flex-col items-center p-5 rounded-xl bg-[#FBF8F3] dark:bg-slate-900">
                                <div className="w-12 h-12 bg-gray-200 dark:bg-slate-800 rounded-xl mb-3" />
                                <div className="h-3 bg-gray-200 dark:bg-slate-800 rounded w-20 mb-2" />
                                <div className="h-2 bg-gray-200 dark:bg-slate-800 rounded w-16" />
                            </div>
                        ))}
                    </div>
                )}
                ...
                {!loading && roomItems.length > 0 && (
                    <div className="mb-12">
                        <h3 className="text-lg font-bold text-[#2A2018] dark:text-white mb-6 text-center">In Your Private Villa</h3>
                            ...
                            {roomItems.map((item, index) => {
                                ...
                                return (
                                    <motion.div
                                        ...
                                        className="flex flex-col items-center p-5 rounded-xl bg-[#FBF8F3] dark:bg-slate-900 text-center">
                                        <div className="w-12 h-12 bg-[#EFF7F3] dark:bg-slate-800 rounded-xl flex items-center justify-center mb-3">
                                            <Icon className="w-6 h-6 text-[#2E5D4B] dark:text-emerald-400" />
                                        </div>
                                        <span className="font-medium text-[#2A2018] dark:text-white text-sm mb-1">{item.label}</span>
                ...
                {propertyItems.length > 0 && (
                    <div>
                        <h3 className="text-lg font-bold text-[#2A2018] dark:text-white mb-6 text-center">Property & Services</h3>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            {propertyItems.map((item, index) => {
                                ...
                                return (
                                    <motion.div
                                        ...
                                        viewport={{ once: true }}
                                        transition={{ delay: index * 0.05 }}
                                        className="flex flex-col items-center p-5 rounded-xl bg-[#2E5D4B]/5 dark:bg-emerald-500/10 text-center">
